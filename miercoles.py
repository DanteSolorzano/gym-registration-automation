...
# --- 3. Abrir la página ---
driver.get(
    "https://www3.ugto.mx/desarrolloestudiantil/clasesdeportivas/calendarioClasesCGTO.php")
...
if len(buttons) >= 5:
    lunes = buttons[-3]
    lunes.click()
    print("Se hizo clic en el botón correspondiente al miercoles")
else:
...
try:
    horario_lunes = wait.until(EC.element_to_be_clickable(
        (By.XPATH,
         '//*[@id="Calendar"]/div/div[2]/div/div[2]/div[9]/span[2]')
    ))
    horario_lunes.click()
    print("Se hizo clic en el botón del horario del miercoles")
except:
...
try:
    # Nombre
    input_nombre = wait.until(EC.visibility_of_element_located(
    ...
    # Campus
    select_campus = Select(driver.find_element(
        By.XPATH, '//*[@id="userCampus"]'))
    select_campus.select_by_index(2)  # opción 3, los índices empiezan en 0
    time.sleep(1)
    # División
    select_division = Select(driver.find_element(
        By.XPATH, '//*[@id="userDivision"]'))
    select_division.select_by_index(2)  # opción 3
    time.sleep(1)

    # Programa
    select_programa = Select(driver.find_element(
        By.XPATH, '//*[@id="userPrograma"]'))
    ...
    # JS click para disparar eventos JS
    ...
    btn_enviar = driver.find_element(
        By.XPATH, '//*[@id="btnUserRegistrarClase"]')
    btn_enviar.click()
